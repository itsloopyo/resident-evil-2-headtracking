...
bool Config::Load(const char* path) {
    ...
    // Read into a full-width int first: casting straight to uint16_t would
    // silently wrap an out-of-range value (e.g. 70000 -> 4464) into a valid
    // but wrong port. Reject anything outside the usable range instead.
    int configuredPort = reader.ReadInt("Network", "UDPPort", udpPort);
    if (configuredPort < 1024 || configuredPort > 65535) {
        Logger::Instance().Warning("UDP port %d out of valid range [1024, 65535], using default %d",
                                   configuredPort, DEFAULT_UDP_PORT);
        udpPort = DEFAULT_UDP_PORT;
    } else {
        udpPort = static_cast<uint16_t>(configuredPort);
    }
...
bool Config::Save(const char* path) const {
    ...
    file << "; RE2 Head Tracking Configuration\n";
