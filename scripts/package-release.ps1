...
# Package RE2 Head Tracking release ZIPs
...
Write-Host "=== RE2 Head Tracking - Package Release ===" -ForegroundColor Magenta
Write-Host ""
...
$dllPath = Join-Path $projectDir "bin/Release/RE2HeadTracking.dll"
...
$vendorZip = Join-Path $vendorSrcDir "RE2.zip"
...
foreach ($vendorFile in @("RE2.zip", "LICENSE", "README.md")) {
...
# submodule state; never fast-forward cameraunlock-core mid-package.
Copy-SharedBundle -StagingDir $ghStagingDir -NoRefresh

$ghZipName = "RE2HeadTracking-v$version-installer.zip"
